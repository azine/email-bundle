        ...
        $rootNode
        	->children()
	        	...
	        	->arrayNode(AzineEmailExtension::NO_REPLY)
	        		->addDefaultsIfNotSet()
                    ->children()
	        		    ->scalarNode	(AzineEmailExtension::NO_REPLY_EMAIL_ADDRESS)	->defaultValue("no-reply@example.com")->cannotBeEmpty()		->info("the no-reply email-address")->isRequired()->end()
	        			->scalarNode	(AzineEmailExtension::NO_REPLY_EMAIL_NAME)		->defaultValue('notification daemon')->cannotBeEmpty()		->info("the name to appear with the 'no-reply'-address.")->isRequired()->end()
	        			->end()
	        		->end()
